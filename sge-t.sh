...
LINK_BIN="https://github.com/sge-network/sge/releases/download/v${BIN_VERSION}/sged-linux-amd64"
SEEDS="e2c5f2a902b7e6b8c006008e962ab4ddd70cdd78@sge.peers-t.stavr.tech:1146"
PEERS=$(curl -sS https://sge.rpc.t.stavr.tech/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | paste -sd, -)
MAX_NUM_INBOUND_PEERS=10
MAX_NUM_OUTBOUND_PEERS=40

SNAP_RPC="https://sge.rpc.t.stavr.tech:443"
SNAPSHOTS_DIR="https://server-1.stavr.tech/Testnet/Sge/"
SNAPSHOT_MASK_NAME1="sge-snap-[0-9]+\.tar\.lz4"
...
WASM_URL="https://server-3.stavr.tech/Testnet/Sge/wasm-sget.tar.lz4"
...
ExecStart=$(which ${BIN_NAME}) start --home ${HOME}/${HOME_FOLDER}
Restart=on-failure
RestartSec=5
LimitNOFILE=65535
